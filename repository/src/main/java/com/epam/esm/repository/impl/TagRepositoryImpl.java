...
public class TagRepositoryImpl implements TagRepository {
    ...
    public List<Tag> findMostWidelyUsedTag() {
        final String mostWidelyUsedTagQuery = "SELECT DISTINCT t1.tag_id as id, t1.tag_name as name " +
                "FROM ((SELECT distinct most_payable_customers.id as customer_id, tag.id as tag_id, " +
                "count(tag.id) as tag_amount, tag.name as tag_name " +
                    "FROM (SELECT distinct orders.user_id as id " +
                        "FROM orders GROUP BY orders.user_id " +
                        "HAVING sum(orders.price) = (SELECT sum(price) OVER (PARTITION BY user_id) " +
                            "FROM orders LIMIT 1)) AS most_payable_customers " +
                    "JOIN orders ON most_payable_customers.id = orders.user_id " +
                    "JOIN order_has_gift_certificate ON orders.id = order_has_gift_certificate.order_id " +
                    "JOIN gift_certificate ON gift_certificate.id = order_has_gift_certificate.gift_certificate_id " +
                    "JOIN gift_certificate_has_tag ON gift_certificate_has_tag.gift_certificate_id = gift_certificate.id " +
                    "JOIN tag ON tag.id = gift_certificate_has_tag.tag_id group by most_payable_customers.id, tag.id)) as t1 " +
                "JOIN (SELECT customer_id, max(tag_amount) as max_tag_amount " +
                    "FROM ((SELECT distinct most_payable_customers.id as customer_id, tag.id as tag_id, " +
                        "count(tag.id) as tag_amount, tag.name as tag_name " +
                        "FROM (SELECT distinct orders.user_id as id " +
                            "FROM orders GROUP BY orders.user_id " +
                    ...
                    "group by most_payable_customers.id, tag.id)) as ttt GROUP BY customer_id) as t2 " +
                "ON t1.customer_id = t2.customer_id and t1.tag_amount = t2.max_tag_amount";
